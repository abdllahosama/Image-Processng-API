...
describe('Test image api responce', () => {
    //check end point with all data
    it('check full data endpoint succrss', async () => {
        const responce = await request.get(
            '/api/image?image=abdo&height=400&width=500'
        )
        expect(responce.status).toBe(200)
    })

    //check end point without image it must fail
    it('check lost data endpoint fail without image', async () => {
    ...
    it('check lost data endpoint fail with width less than 1', async () => {
        const responce = await request.get(
    ...
    it('check lost data endpoint fail with height less than 1', async () => {
        const responce = await request.get(
